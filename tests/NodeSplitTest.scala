...
class NodeSplitTest extends FunSuite {
  ...
  test("FeatureValueAndData Equal") {
    assert(new FeatureValueAndData(new FeatureValue(0), 1, 2.0) ===
      new FeatureValueAndData(new FeatureValue(0), 1, 2.0))
  }
  
  test("FeatureValueAndData Accumulate Same Type") {
    val data = Array(
        new FeatureValueAndData(new FeatureValue(0), 1, 2.0),
        new FeatureValueAndData(new FeatureValue(3), 2, 1.0),
        new FeatureValueAndData(new FeatureValue(6), 4, 3.0),
        new FeatureValueAndData(new FeatureValue(7), 3, 6.0),
        new FeatureValueAndData(new FeatureValue(9), 5, 4.0))
    val accumulatedData = data.scanLeft(new FeatureValueAndData(
        data(0).featureValue, 0, 0))(_ accumulate _)
    val expectedData = Array(
        new FeatureValueAndData(new FeatureValue(0), 0, 0.0),
        new FeatureValueAndData(new FeatureValue(0), 1, 2.0),
        new FeatureValueAndData(new FeatureValue(3), 3, 3.0),
        new FeatureValueAndData(new FeatureValue(6), 7, 6.0),
        new FeatureValueAndData(new FeatureValue(7), 10, 12.0),
        new FeatureValueAndData(new FeatureValue(9), 15, 16.0)) 
    assert(expectedData === accumulatedData)
  }
  
  test("FeatureValueData Equal") {
    assert(new FeatureValueData(1, 2) === new FeatureValueData(1, 2))
  }
  ...
  test("FeatureValueData Accumulate Y") {
    ...
    val expectedMap = Map(
        new FeatureValue(1) -> new FeatureValueData(2, 8),
        new FeatureValue(3) -> new FeatureValueData(1, 5),
        new FeatureValue(4) -> new FeatureValueData(2, 15))
  ...
  def checkBestSplit(expectedBestSplit: BestSplit,
    ...
    val nodeSplit = new NodeSplit(0, featuresType)
